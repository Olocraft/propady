...
const CryptoPayment = ({
  ...
  const [paymentStep, setPaymentStep] = useState<"select" | "pay" | "confirm">(
    "select"
  );
  ...
  const handleCopy = () => {
    ...
    toast.success("The wallet address has been copied to your clipboard.");
  };
  ...
  const mockConfirmPayment = async () => {
    ...
    if (!propertyId) {
      toast("Could not identify the property for this transaction");
      return;
    }
    ...
    try {
      ...
      if (verified) {
        ...
        if (success) {
          ...
          toast.success(
            `You've successfully paid ${getCryptoAmount()} ${
              selectedCrypto.symbol
            }`
          );
        }
      ...
      <DialogContent className="sm:max-w-md bg-background border-white/10 text-white">
        <DialogHeader>
          ...
          <DialogDescription className="text-white/70">
            {paymentStep === "select" &&
              "Choose your preferred cryptocurrency for this transaction"}
            {paymentStep === "pay" &&
              `Send exactly ${getCryptoAmount()} ${
                selectedCrypto.symbol
            ...
            <div className="p-4 bg-black/20 rounded-lg space-y-2">
              <div className="flex justify-between items-center">
                ...
                <Button
                  ...
                  size="sm"
                  onClick={handleCopy}
                  className="h-8 border-white/10 text-white/80"
                >
                  {copied ? (
              ...
              <p className="text-sm text-white/70 font-medium">
                Amount: {getCryptoAmount()} {selectedCrypto.symbol}
              </p>
            </div>

            <div className="text-center">
              <p className="text-sm text-white/70 mb-4">
                After sending, click below to confirm your payment
              </p>
              <Button
